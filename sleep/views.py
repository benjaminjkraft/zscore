...
@login_required
def mysleep(request):
    return render_to_response('sleep/mysleep.html', {},
...
def editOrCreateSleep(request, sleep = None, success = False):
    context = {'success': success}
    if "from" in request.GET and request.GET["from"] == "partial":
        context["fromPartial"] = True
    prof      = request.user.sleeperprofile
    defaulttz = prof.timezone
    ...
    if sleep: # we're editing a sleep
        ...
        context.update({"start": s.start_local_time().strftime(fmt),
                        "end"  : s.end_local_time().strftime(fmt),
                        "tz"   : s.timezone})
    else: # we're creating a new sleep
        if request.method == 'POST':
            form = SleepForm(request.user, fmt, request.POST, instance = Sleep(user = request.user))
        else:
            today = now().astimezone(pytz.timezone(defaulttz)).replace(hour = 0,
                                                                       minute = 0,
                                                                       second = 0,
                                                                       microsecond = 0)
            initial = {
            ...
            if "error" in request.GET:
                if request.GET["error"] == "partial":
                    ...
                    try:
                        ...
                        initial.update({"timezone"  : p.timezone,
                                        "start_time": p.start_local_time().strftime(fmt)})
...
def graph(request):
    context = {
               "user"  : request.user,
               "sleeps": request.user.sleep_set.all().order_by('-end_time')
              }
    return render_to_response('graph.html', context, context_instance = RequestContext(request))
...
def groups(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            gs = SleeperGroup.objects.filter(
                                             name__icontains = form.cleaned_data['group'],
                                             privacy__gte = SleeperGroup.REQUEST
...
def createGroup(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            g=form.save()
            m=Membership(user    = request.user,
                         group   = g,
                         privacy = request.user.sleeperprofile.privacyLoggedIn,
...
def groupJoin(request):
    if 'group' in request.POST:
        ...
        if gs.count() != 1:
            raise Http404
        g = gs[0]
        if g.privacy < SleeperGroup.PUBLIC:
            raise PermissionDenied
        m = Membership(user    = request.user,
                       group   = g,
                       privacy = request.user.sleeperprofile.privacyLoggedIn)
...
def manageGroup(request,gid):
    ...
    if request.method == 'POST' and "SleeperSearchForm" in request.POST:
        ...
        if searchForm.is_valid():
            us=User.objects.filter(
                                   username__icontains=searchForm.cleaned_data['username']
                                  ).exclude(sleepergroups__id=g.id)
            context['results']     = us
    ...
    if context['isAdmin']:
        ...
        if 'page' not in context and context['requests'].count() > 0:
            # If we don't have a default page already to go to, and we have requests, load request page first.
            context['page'] = 3
...
def leaderboard(request, group = None):
    ...
    metricNames = [m.name for m in userMetrics]
    if 'sort' not in request.GET or request.GET['sort'] not in metricNames:
        sortBy = 'zScore'
    else:
    ...
    numLeaderboard = len(filter(lambda s: s['rank'] != 'n/a', ss))
    n = now()
    recentWinner = Sleeper.objects.bestByTime(start = (n - datetime.timedelta(3)),
                                              end   = n,
                                              user  = request.user,
                                              group = group
                                             )[0]
...
def creep(request, username = None):
    if not username:
        ...
        try:
            ...
            if not(request.user.is_anonymous()) and request.user.pk == user.pk:
                context["isself"] =True
        ...
        if priv>=p.PRIVACY_GRAPHS:
            if "type" in request.GET and request.GET["type"] == "graph":
                return render_to_response('graph.html', context, context_instance = RequestContext(request))
            context["graphs"] = True
...
def createSleep(request):
    ...
    if start > end:
        start,end = end, start #if end is after start, flip them
    s = Sleep(user       = request.user,
              start_time = start,
              end_time   = end,
              comments   = comments,
              date       = date,
...
def getSleepsJSON(request):
    ...
    for sleep in sleeps:
        ...
        # warning: the following is kind of hacky but it's better than dealing with the timezones in JS.
        # JS doesn't understand timezones, so we convert the timezone server-side, then pass it through to JS
        # without telling the JS what timezone it's in.  JS interprets it as local time, which is slightly
        # incorrect but works since all we want to do is get the hours/minutes/seconds back out as local time.
